...
func (s *PublicSfcAPI) GetDelegationClaimedRewards(ctx context.Context, addr common.Address, stakerID hexutil.Uint) (*hexutil.Big, error) {
	v, err := s.b.GetDelegationClaimedRewards(ctx, sfctype.DelegatorID{addr, idx.StakerID(stakerID)})
	if err != nil {
		return nil, err
	}
	return (*hexutil.Big)(v), err
...
func (s *PublicSfcAPI) addDelegatorMetricFields(ctx context.Context, res map[string]interface{}, id sfctype.DelegatorID) (map[string]interface{}, error) {
...
// RPCMarshalDelegator converts the given delegation to the RPC output .
func RPCMarshalDelegator(it sfctype.SfcDelegationAndID) map[string]interface{} {
	return map[string]interface{}{
...
func (s *PublicSfcAPI) GetDelegationsOf(ctx context.Context, stakerID hexutil.Uint64, verbosity hexutil.Uint64) ([]interface{}, error) {
	...
	for i, it := range delegations {
		delegationRPC := RPCMarshalDelegator(it)
		if verbosity >= 2 {
			delegationRPC, err = s.addDelegatorMetricFields(ctx, delegationRPC, it.ID)
			if err != nil {
				return nil, err
			}
		}
...
func (s *PublicSfcAPI) GetDelegation(ctx context.Context, addr common.Address, stakerID hexutil.Uint, verbosity hexutil.Uint64) (map[string]interface{}, error) {
	id := sfctype.DelegatorID{addr, idx.StakerID(stakerID)}
	...
	return s.addDelegatorMetricFields(ctx, delegationRPC, id)
}
