...
type (
	...
	ValidatorsBuilder map[idx.StakerID]pos.Stake
...
func (vv ValidatorsBuilder) Set(id idx.StakerID, stake pos.Stake) {
	if stake == 0 {
		delete(vv, id)
	} else {
		vv[id] = stake
...
func EqualStakeValidators(ids []idx.StakerID, stake pos.Stake) *Validators {
	builder := NewBuilder()
	for _, id := range ids {
...
func ArrayToValidators(ids []idx.StakerID, stakes []pos.Stake) *Validators {
	builder := NewBuilder()
	for i, id := range ids {
...
func (vv *Validators) Get(id idx.StakerID) pos.Stake {
	return vv.values[id]
}
...
func (vv *Validators) GetStakeByIdx(i idx.Validator) pos.Stake {
...
// The order is the same as for Idxs().
func (vv *Validators) SortedStakes() []pos.Stake {
...
func (vv *Validators) sortedArray() pos.validators {
	array := make(pos.validators, 0, len(vv.values))
	for id, s := range vv.values {
		array = append(array, pos.validator{
			ID:    id,
			Stake: s,
		})
	}
...
func (vv *Validators) Quorum() pos.Stake {
	return vv.TotalStake()*2/3 + 1
}

// TotalStake of validators.
func (vv *Validators) TotalStake() (sum pos.Stake) {
...
func (gv GValidators) Validators() *Validators {
	...
	for _, validator := range gv {
		builder.Set(validator.ID, pos.BalanceToStake(validator.Stake))
	}
	return builder.Build()
}
