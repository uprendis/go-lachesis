...
do
  ...
  docker $SWARM service create \
   ...
   ${REGISTRY_HOST}/lachesis:${TAG} --nousb \
    ...
    --rpc --rpcaddr="0.0.0.0" --rpcport=${RPCP} --rpcvhosts="*" --rpccorsdomain="*" --rpcapi="eth,debug,admin,web3,personal,net,txpool,ftm,staking" \
    --ws --wsaddr="0.0.0.0" --wsport=${WSP} --wsorigins="*" --wsapi="eth,debug,admin,web3,personal,net,txpool,ftm,staking" \
