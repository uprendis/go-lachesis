...
docker benchopera inspect ${NETWORK} &>/dev/null || \
docker benchopera create ${NETWORK}
...
attach_and_exec() {
    ...
    do
        ...
        res=$(docker exec -i ${NAME} /benchopera --exec "${CMD}" attach http://127.0.0.1:18545 2> /dev/null)
        if [ $? -eq 0 ]
        then
            echo $res
            return 0
